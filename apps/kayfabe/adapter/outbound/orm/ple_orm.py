...
class PleEventModel(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, default=PleEventStatus.UPCOMING)
...
class PleMatchModel(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, default=PleMatchStatus.SCHEDULED)
...
class PlePredictionModel(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    match_id: Mapped[int] = mapped_column(ForeignKey("ple_matches.id", ondelete="CASCADE"), index=True)
