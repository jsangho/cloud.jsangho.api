...
def event_to_snapshot(event: PleEventModel) -> PleEventSnapshotDto:
    return PleEventSnapshotDto(
        id=event.id,
        slug=event.slug,
        status=event.status,
        finished_at=event.finished_at,
        matches=[
            PleMatchSnapshotDto(id=m.id, match_key=m.match_key, status=m.status)
            for m in event.matches
        ],
    )


def match_to_read(row: PleMatchModel) -> PleMatchReadDto:
    return PleMatchReadDto(
        id=row.id,
...
def event_to_read(event: PleEventModel) -> PleEventReadDto:
    return PleEventReadDto(
        slug=event.slug,
        label=event.label,
        month=event.month,
        year=event.year,
