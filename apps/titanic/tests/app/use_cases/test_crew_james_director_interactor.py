...
def mock_repository():
    ...
    repo.introduce_myself = AsyncMock(
        return_value=JamesDirectorResponse(id=40000, name="James Cameron이 레포지토리에 다녀옴")
    )
...
class TestIntroduceMyself:
    ...
    def test_returns_repository_response(self, interactor):
        response = asyncio.run(interactor.introduce_myself(JamesDirectorQuery(id=4, name="James Cameron")))
...
class TestUploadTitanicFile:
    def test_creates_one_passenger_command_per_record(self, interactor, mock_repository):
        asyncio.run(interactor.upload_titanic_file([_record(passenger_id="1"), _record(passenger_id="2")]))

        person_commands = mock_repository.upload_titanic_file.call_args.kwargs["person_commands"]
        assert len(person_commands) == 2

    def test_passenger_command_contains_correct_fields(self, interactor, mock_repository):
        asyncio.run(interactor.upload_titanic_file([_record(passenger_id="7", gender="female", age="28")]))
    ...
    def test_booking_command_contains_correct_fields(self, interactor, mock_repository):
        asyncio.run(interactor.upload_titanic_file([_record(pclass="1", fare="100.0", embarked="C")]))

        booking_commands = mock_repository.upload_titanic_file.call_args.kwargs["booking_commands"]
